...
def _make_svc(tmp_path, monkeypatch, llm):
    ...
    monkeypatch.setattr(UapisDataSource, "get_weather", fake_weather)
    svc = AgentService()
    svc.llm = llm
...
async def test_critic_triggers_on_hard_data(tmp_path, monkeypatch):
    ...
    llm = ScriptedStreamLLM([
        [{"type": "end",
          "tool_calls": [{"id": "1", "function": {"name": "weather_query", "arguments": '{"city":"成都"}'}}]}],
...
async def test_critic_skips_without_hard_data(tmp_path, monkeypatch):
    ...
    llm = ScriptedStreamLLM([
        [{"type": "end", "tool_calls": [
            {"id": "1", "function": {"name": "poi_search", "arguments": '{"keywords":"景点","city":"成都"}'}}]}],
