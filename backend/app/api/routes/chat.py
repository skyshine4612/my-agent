...
# SSE 流式对话路由：POST /api/agent/chat，把 AgentService.plan_stream 产出的事件流
...
async def chat(req: ChatRequest, x_user_id: str = Header(default="anonymous")):
    """SSE 流式对话端点：接收对话请求，逐事件推送规划/执行/整合结果。

    x_user_id 从请求头 X-User-Id 解析，用于用户隔离；未传时兜底 anonymous。
    """

    async def gen():
        """事件生成器：把 plan_stream 的每个事件 dict 转成 SSE 的 {event, data} 结构。"""
        # 逐条消费编排 service 产出的事件流，event 用 type 字段、data 用 ensure_ascii=False 保留中文
        async for ev in service.plan_stream(x_user_id, req.conversation_id, req.message):
