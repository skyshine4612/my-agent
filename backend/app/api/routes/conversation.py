...
# 会话管理路由：GET/POST /api/conversations、GET /api/conversations/{id}，
...
async def list_conversations(x_user_id: str = Header(default="anonymous")):
    """列出当前用户的所有会话（按创建时间倒序）。"""
    return service.conv.list_conversations(x_user_id)


@router.post("/conversations")
async def create_conversation(body: ConversationCreate, x_user_id: str = Header(default="anonymous")):
    """新建一个会话，返回会话 id。"""
    cid = await service.conv.create_conversation(x_user_id, body.title)
...
async def get_conversation(conv_id: str, x_user_id: str = Header(default="anonymous")):
    ...
    return await service.conv.get_history(x_user_id, conv_id)


@router.delete("/conversations/{conv_id}")
async def delete_conversation(conv_id: str, x_user_id: str = Header(default="anonymous")):
    """删除指定会话及其消息；归属校验失败返回 deleted=False。"""
    ok = await service.conv.delete_conversation(x_user_id, conv_id)
    return {"deleted": ok}
