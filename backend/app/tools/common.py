...
def make_weather_query(ds):
    """天气查询工具：查城市未来 N 天天气（UAPIS 数据源，最多 7 天预报）。"""

    async def weather_query(city, days=7):
        """查询指定城市未来 N 天（默认 7 天）的天气，返回天气列表。"""
...
def register_common_tools(registry):
    ...
    全部通用能力（含天气）共用一个 UAPIS 数据源；高德数据源仅在旅行工具（travel.py）中使用。
    """
    ...
    registry.register("translate", "翻译文本，自动检测源语言，翻译成目标语言",
                      {"type": "object", "properties": {
                          ...
                          "to_lang": {"type": "string",
                                      "description": "目标语言代码，如 en(英语)/zh(中文)/ja(日语)/ko(韩语)，默认 en"},
                      }, "required": ["text"]},
    ...
    registry.register("recipe_search", "按菜名或食材关键词搜索菜谱，返回菜谱列表（含 id）",
                      {"type": "object", "properties": {
                          "keyword": {"type": "string", "description": "菜名或食材关键词，如 宫保鸡丁、鸡胸肉"}},
                       "required": ["keyword"]},
